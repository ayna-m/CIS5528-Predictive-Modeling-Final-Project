...
def eval(model, X_test, y_test, name):
    ...
    with torch.no_grad():
        ...
        return [precision, recall, specificity, sensitivity, accuracy]


def model_graph(model, num_features, name):
    input_tensor = torch.randn(1, num_features)
    draw_graph(model, input_tensor,
               hide_inner_tensors =True, save_graph=True,
               filename=f'plots/{name}_model_graph')
