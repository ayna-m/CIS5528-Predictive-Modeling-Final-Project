...
clinical_raw.columns = clinical_raw.columns.astype(str)
del clinical_raw['ID']
del clinical_raw['group']
...
# Create data loaders
clinical_test = clinical_raw.sample(int(len(clinical_raw)*0.5))
clinical_train = clinical_raw.sample(int(len(clinical_raw)*0.25))
clinical_val = clinical_raw.sample(int(len(clinical_raw)*0.25))
...
all_data = Dataset_fix(pd.concat([clinical_test, clinical_train, clinical_val]))
...
def test(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            batch_loss = loss_fn(pred, y.view(-1,1).float())
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.show()
_, preds, targets = test(val_dataloader, model, loss_fn)
